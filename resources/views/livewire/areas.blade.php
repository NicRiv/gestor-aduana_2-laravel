<div>
    ...
    <div class="flex justify-between px-5 py-5 bg-slate-200">
        <div class="flex items-center justify-start gap-x-2">
            <h4 class="font-medium leading-tight text-2xl mt-0 mb-2">Áreas ></h4>
            <select wire:model="search_div">
                <option value="">Depósitos</option>
                @foreach ($lugardeposito as $lugar)
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="order('deposito')">
                                Depósito
                            </th>
                            <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="order('comentario')">
                                Comentario
                            </th>
                    ...
                    <tbody>
                        @foreach ($areadeposito as $area)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="block text-center {{ $area['activo'] ? 'block px-2 bg-lime-400 text-white' : '' }}">
                                        {{ $area['activo'] ? 'Si' : 'No' }}
                                    </span>
                                </td>
                            </tr>
    ...
    <x-jet-dialog-modal wire:model="open">
        ...
        <x-slot name="content">
            <div class="mb-4">
                <x-jet-label value="Nombre / Código Área" />
