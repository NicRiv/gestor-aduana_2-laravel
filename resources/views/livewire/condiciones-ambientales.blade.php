<div>
    ...
    <div class="flex justify-between px-5 py-5 bg-slate-200">
        <div class="flex items-center justify-start gap-x-2">
            <h4 class="font-medium leading-tight text-2xl mt-0 mb-2">Condiciones ambientales</h4>
        </div>
        <div class="flex items-center gap-x-4">
            <x-jet-secondary-button>
                Lista
            </x-jet-secondary-button>
            @can('Crear Condición Ambiental')
    ...
    <div class="flex justify-between px-5 py-2 bg-white border-y-2 border-slate-400">
        Inicio / {Condiciones ambientales}
    </div>
                    ...
                    <tbody>
                        @foreach ($ambientes as $ambiente)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="block text-center {{ $ambiente['activo'] ? 'block px-2 bg-lime-400 text-white' : '' }}">
                                        {{ $ambiente['activo'] ? 'Si' : 'No' }}
                                    </span>
                                </td>
                            </tr>
                        @endforeach
    ...
    <x-jet-dialog-modal wire:model="open">
        ...
        <x-slot name="content">
            <div class="mb-4">
                <x-jet-label value="Nombre Ambiente" />
