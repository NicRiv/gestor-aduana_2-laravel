<div>
    ...
    <div class="flex justify-between px-5 py-5 bg-slate-200">
        <div class="flex items-center justify-start gap-x-2">
            <h4 class="font-medium leading-tight text-2xl mt-0 mb-2">Tipo de ingresos / egresos</h4>
        </div>
        <div class="flex items-center gap-x-4">
            <x-jet-secondary-button>
                Lista
            </x-jet-secondary-button>
            @can('Crear Tipo ingreso/egreso')
    ...
    <div class="flex justify-between px-5 py-2 bg-white border-y-2 border-slate-400">
        Inicio / {Tipo de ingresos / egresos}
    </div>
    ...
    <div class="m-5 p-5 rounded-lg shadow-lg bg-slate-200">
        ...
        @if ($tipoingresoegreso->count())
            ...
            <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="order('metodo')">
                                Método
                            </th>
                    ...
                    <tbody>
                        @foreach ($tipoingresoegreso as $tipo_ie)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="block text-center {{ $tipo_ie['activo'] ? 'block px-2 bg-lime-400 text-white' : '' }}">
                                        {{ $tipo_ie['activo'] ? 'Si' : 'No' }}
                                    </span>
                                </td>
                            </tr>
                        @endforeach
                ...
                @if ($tipoingresoegreso->hasPages())
                    <div class="p-2 bg-gray-50">
                        {{ $tipoingresoegreso->links() }}
                    </div>
                @endif
            </div>
